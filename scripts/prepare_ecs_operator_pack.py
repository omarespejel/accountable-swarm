...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "operator_proof_required": [
            ...
            "runs/ecs/ecs_proof_review.md generated with prepare-ecs-proof-review",
        ],
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "   The report must include `qwenguard-memory-fixture`,",
            "   `qwen-vl-fixture_model_qwen3-vl-flash`,",
            "   `deployed_commit_matches_collector_head`,",
            "   `ecs_metadata_identity_document_present`,",
            "   `ecs_metadata_region_matches`, `ecs_metadata_instance_id_matches`,",
            "   and `ecs_metadata_public_ip_matches`.",
            ...
            "8. Generate the human review note after the smoke report and terminal",
            "   artifact exist:",
            "",
            "   ```bash",
...
def _render_commands(*, base_url: str, commit: str) -> str:
    ...
    return "\n".join(
        [
            ...
            'QWEN_VL_MODEL="${QWEN_VL_MODEL:-$(env_value QWEN_VL_MODEL)}"',
            'if [ -z "${QWEN_VL_MODEL}" ]; then',
            '  QWEN_VL_MODEL="qwen3-vl-flash"',
            "fi",
            'if [ -z "${BASE_URL}" ]; then',
