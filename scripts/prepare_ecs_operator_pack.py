...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "operator_proof_required": [
            ...
            "public base URL used for smoke capture",
            "deployed commit SHA",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "4. Set `BASE_URL` to the Alibaba ECS public IP or DNS for proof capture.",
            "   Do not rely on loopback-only smoke output for the final artifact.",
            "5. Copy `.env.template` to `.env` on the ECS host and fill the",
            "   Alibaba key there only.",
            "6. Run `operator_commands.sh` from the repository root.",
            "7. Save `runs/ecs/ecs_smoke_report.json` only if it contains",
            "   `\"outcome\":\"GO\"`.",
            "8. Record region, OS image, public base URL, deployed commit, command",
            "   transcript, and a terminal screenshot showing the proof ran on",
            "   Alibaba ECS.",
            "",
            "## Code File Links",
            "",
