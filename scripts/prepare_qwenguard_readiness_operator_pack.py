...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "remaining_go_gates": [
            ...
            "human-reviewed Alibaba ECS proof note generated from that report",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "",
            "```bash",
            f"bash {_display_path(repo_root, files['commands'])} next-steps",
            f"bash {_display_path(repo_root, files['commands'])} all-preflight",
            "```",
            "",
            "`next-steps` only prints the remaining operator sequence and",
            "expected artifact paths. `all-preflight` is the locally",
            "test-covered no-camera/no-ECS-host preflight. It does not invoke",
            "camera capture or ECS proof collection; it generates the physical,",
            "ECS, and submission packs, runs the existing physical-pack",
            "`all-safe` phase, and confirms the current audit remains",
            "`NARROW_CLAIM` for missing operator evidence.",
            "",
            ...
            "Also copy or record a terminal/screenshot/video artifact proving the",
            "public ECS context. Then create the ECS proof review note:",
            "",
            "```bash",
            "QWENGUARD_ECS_REVIEWED_BY=\"REPLACE_WITH_HUMAN_REVIEWER\" \\",
            "QWENGUARD_ECS_REVIEW_DATE=\"YYYY-MM-DD\" \\",
            "QWENGUARD_ECS_TERMINAL_ARTIFACT=\"runs/ecs/ecs_terminal_proof.txt\" \\",
            f"bash {_display_path(repo_root, files['commands'])} ecs-review",
            "```",
            "",
            "Create the final video review only after a human has watched the",
            "actual final cut and the ECS proof review exists:",
            "",
            "```bash",
            "QWENGUARD_REVIEWED_BY=\"REPLACE_WITH_HUMAN_REVIEWER\" \\",
            "QWENGUARD_REVIEW_DATE=\"YYYY-MM-DD\" \\",
            ...
            "- `runs/ecs/ecs_proof_review.md` with explicit human review of",
            "  Alibaba context, public endpoint, deployed commit, security group,",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            'ECS_PROOF_REVIEW="${QWENGUARD_ECS_PROOF_REVIEW:-runs/ecs/ecs_proof_review.md}"',
            'ECS_REPORT="${QWENGUARD_ECS_REPORT:-runs/ecs/ecs_smoke_report.json}"',
            'ECS_TERMINAL_ARTIFACT="${QWENGUARD_ECS_TERMINAL_ARTIFACT:-}"',
            'VIDEO_ARTIFACT="${QWENGUARD_VIDEO_ARTIFACT:-${QWENGUARD_VIDEO_ARTIFACT_DEFAULT}}"',
            'SO101_RUN_DIR="${QWENGUARD_GO_RUN_DIR:-runs/physical/qwenguard_physical_go}"',
            "",
            "repo_relative_guard() {",
            "  case \"$1\" in /*|..|../*|*/..|*/../*) echo \"$2 must be repo-relative and must not contain ..\" >&2; exit 2 ;; esac",
            "}",
            "secret_material_guard() {",
            "  local value=\"$1\"",
            "  local name=\"$2\"",
            "  if [[ -z \"${value}\" ]]; then",
            "    return 0",
            "  fi",
            "  local github_prefix=\"github\"",
            "  local gh_prefix=\"gh\"",
            "  local sk_prefix=\"sk\"",
            "  case \"${value}\" in",
            "    *ALIBABA_API_KEY*|*\"${github_prefix}\"_pat_*|*\"${gh_prefix}\"p_*|*\"${gh_prefix}\"o_*|*\"${gh_prefix}\"u_*|*\"${gh_prefix}\"s_*|*\"${gh_prefix}\"r_*|*\"${sk_prefix}\"-*)",
            "      echo \"${name} must not contain secret-like material\" >&2",
            "      exit 2",
            "      ;;",
            "  esac",
            "  local lower_value",
            ...
            "repo_relative_guard \"${ECS_PROOF_REVIEW}\" QWENGUARD_ECS_PROOF_REVIEW",
            "repo_relative_guard \"${ECS_REPORT}\" QWENGUARD_ECS_REPORT",
            "repo_relative_guard \"${SO101_RUN_DIR}\" QWENGUARD_GO_RUN_DIR",
            "secret_material_guard \"${ECS_TERMINAL_ARTIFACT}\" QWENGUARD_ECS_TERMINAL_ARTIFACT",
            "secret_material_guard \"${VIDEO_ARTIFACT}\" QWENGUARD_VIDEO_ARTIFACT",
            "if [[ -n \"${ECS_TERMINAL_ARTIFACT}\" ]]; then",
            "  case \"${ECS_TERMINAL_ARTIFACT}\" in",
            "    https://*) : ;;",
            "    *) repo_relative_guard \"${ECS_TERMINAL_ARTIFACT}\" QWENGUARD_ECS_TERMINAL_ARTIFACT ;;",
            "  esac",
            "fi",
            ...
            "  ecs-review        Write the human ECS proof review note after explicit env vars.",
            ...
            "  next-steps        Print the remaining operator sequence and expected artifacts.",
            ...
            "}",
            "run_ecs_review() {",
            "  require_env QWENGUARD_ECS_REVIEWED_BY",
            "  require_env QWENGUARD_ECS_REVIEW_DATE",
            "  require_env QWENGUARD_ECS_TERMINAL_ARTIFACT",
            "  python3 -m scripts.prepare_ecs_proof_review \\",
            "    --out \"${ECS_PROOF_REVIEW}\" \\",
            "    --ecs-report \"${ECS_REPORT}\" \\",
            "    --terminal-artifact \"${ECS_TERMINAL_ARTIFACT}\" \\",
            "    --reviewed-by \"${QWENGUARD_ECS_REVIEWED_BY}\" \\",
            "    --review-date \"${QWENGUARD_ECS_REVIEW_DATE}\" \\",
            "    --confirm-report-go \\",
            ...
            "    --confirm-secrets \\",
            "    ${QWENGUARD_ECS_PROOF_REVIEW_OVERWRITE:+--allow-overwrite}",
            "}",
            "run_video_review() {",
            ...
            "}",
            "print_next_steps() {",
            "  local self_cmd=\"${PACK_DIR}/operator_commands.sh\"",
            "  case \"${self_cmd}\" in",
            "    \"${REPO_ROOT}\"/*) self_cmd=\"${self_cmd#${REPO_ROOT}/}\" ;;",
            "  esac",
            "  local self_cmd_q",
            "  printf -v self_cmd_q '%q' \"${self_cmd}\"",
            "  local terminal_artifact=\"${ECS_TERMINAL_ARTIFACT:-runs/ecs/ecs_terminal_proof.txt}\"",
            "  local terminal_artifact_q video_artifact_q trial_csv_q trial_trace_dir_q trial_summary_q ecs_report_q ecs_review_q final_video_review_q so101_capture_report_q",
            "  printf -v terminal_artifact_q '%q' \"${terminal_artifact}\"",
            "  printf -v video_artifact_q '%q' \"${VIDEO_ARTIFACT}\"",
            "  printf -v trial_csv_q '%q' \"${TRIAL_CSV}\"",
            "  printf -v trial_trace_dir_q '%q' \"${TRIAL_TRACE_DIR}\"",
            "  printf -v trial_summary_q '%q' \"${TRIAL_SUMMARY}\"",
            "  printf -v ecs_report_q '%q' \"${ECS_REPORT}\"",
            "  printf -v ecs_review_q '%q' \"${ECS_PROOF_REVIEW}\"",
            "  printf -v final_video_review_q '%q' \"${FINAL_VIDEO_REVIEW}\"",
            "  printf -v so101_capture_report_q '%q' \"${SO101_RUN_DIR}/so101_capture_report.json\"",
            "  cat <<EONEXT",
            "Next operator sequence:",
            "",
            "1. Hardware evidence on the supervised SO-101 machine:",
            "   bash ${self_cmd_q} so101-camera",
            "   QWENGUARD_TRIAL_ID=trial-001 bash ${self_cmd_q} record-success",
            "   QWENGUARD_TRIAL_ID=trial-cloud-hold-001 bash ${self_cmd_q} record-cloud-hold",
            "   bash ${self_cmd_q} summarize-trials",
            "",
            "2. Alibaba ECS evidence on the ECS host, then copy sanitized artifacts back:",
            "   ${ecs_report_q}",
            "   ${terminal_artifact_q} or HTTPS screenshot/video artifact",
            "",
            "3. Human review notes after evidence exists:",
            "   QWENGUARD_ECS_REVIEWED_BY=YOUR_NAME QWENGUARD_ECS_REVIEW_DATE=YYYY-MM-DD QWENGUARD_ECS_TERMINAL_ARTIFACT=${terminal_artifact_q} bash ${self_cmd_q} ecs-review",
            "   QWENGUARD_REVIEWED_BY=YOUR_NAME QWENGUARD_REVIEW_DATE=YYYY-MM-DD QWENGUARD_VIDEO_ARTIFACT=${video_artifact_q} bash ${self_cmd_q} video-review",
            "   bash ${self_cmd_q} audit-final",
            "",
            "Expected evidence paths:",
            "   ${so101_capture_report_q}",
            "   ${trial_trace_dir_q}/*.json",
            "   ${trial_csv_q}",
            "   ${trial_summary_q}",
            "   ${ecs_report_q}",
            "   ${ecs_review_q}",
            "   ${final_video_review_q}",
            "",
            "No phase here asks for raw secrets. ECS secrets belong only in the ECS host .env.",
            "EONEXT",
            "}",
            "run_audit() {",
            "  local allow_flag=\"${1:-}\"",
            ...
            "    --ecs-report \"${ECS_REPORT}\" \\",
            "    --ecs-proof-review \"${ECS_PROOF_REVIEW}\" \\",
