...
DEFAULT_TRIAL_TRACE_DIR = "runs/physical/qwenguard_trials/traces"
DEFAULT_TRIAL_CSV = "runs/physical/qwenguard_trials/trial_results.csv"
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "operator_proof_required": [
            ...
            "manual motion checklist reviewed before any separate LeRobot motion command",
            "measured trial recorder writes trace-bound CSV rows after physical attempts",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"- Measured trial CSV: `{DEFAULT_TRIAL_CSV}`",
            f"- Measured trial traces: `{DEFAULT_TRIAL_TRACE_DIR}`",
            ...
            "- Interlock boundary: this pack does not yet provide a programmatic",
            "  ALLOW-to-LeRobot actuation interlock. Treat fixture/degraded",
            "  DecisionTraces as evidence only, not as automatic permission to move.",
            "",
            "## Motion Interlock Status",
            "",
            "The `fixture` and `degraded` phases can show `ALLOW` or `HOLD` inside",
            "verified no-motion traces. Those traces do not command the SO-101.",
            "",
            "The `training-pack` phase emits a separate LeRobot runbook with manual",
            "operator commands. Until a guarded actuator wrapper binds a fresh",
            "`evaluate_outcome_gate(...) == ALLOW` result and",
            "`PhysicalNodeSafety.assert_safe_for_motion()` to the immediate motion",
            "command, do not call any LeRobot rollout a programmatically gated",
            "QwenGuard motion run.",
            "",
            "Before any human chooses to run the separate LeRobot motion commands,",
            "the operator must confirm all four hardware safeguards:",
            "",
            "- Emergency stop path is reachable and tested.",
            "- Autonomous rollout keeps the SO-101 leader detached or",
            "  non-authoritative.",
            "- Low-speed mode is selected.",
            "- Workspace bounds are physically marked and kept clear.",
            "",
            "The `camera` phase remains safe to run before this interlock exists:",
            "it uses the trace-only camera probe and does not move hardware.",
            "",
            "## Operator Phases",
            "",
            ...
            "- Use `record-qwenguard-trial` through this pack so each measured",
            "  CSV row is explicitly operator-attested and bound to its verified",
            "  `decisiontrace.v2` summary SHA.",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f'TRIAL_TRACE_DIR="${{QWENGUARD_TRIAL_TRACE_DIR:-{DEFAULT_TRIAL_TRACE_DIR}}}"',
            f'TRIAL_CSV="${{QWENGUARD_TRIAL_CSV:-{DEFAULT_TRIAL_CSV}}}"',
            'TRIAL_REPORT_DIR="${QWENGUARD_TRIAL_REPORT_DIR:-runs/physical/qwenguard_trials/reports}"',
            'CONTROL_LABEL="${QWENGUARD_CONTROL_LABEL:-TELEOP}"',
            ...
            'case "${TRIAL_TRACE_DIR}" in /*|..|../*|*/..|*/../*) echo "QWENGUARD_TRIAL_TRACE_DIR must be repo-relative and must not contain .." >&2; exit 2 ;; esac',
            'case "${TRIAL_CSV}" in /*|..|../*|*/..|*/../*) echo "QWENGUARD_TRIAL_CSV must be repo-relative and must not contain .." >&2; exit 2 ;; esac',
            'case "${TRIAL_REPORT_DIR}" in /*|..|../*|*/..|*/../*) echo "QWENGUARD_TRIAL_REPORT_DIR must be repo-relative and must not contain .." >&2; exit 2 ;; esac',
            "",
            ...
            "  record-cloud-hold Record one degraded-cloud HOLD trial.",
            ...
            "contains separate operator-reviewed LeRobot collection/eval commands.",
            "Those commands are manual and are not guarded by a QwenGuard ALLOW-to-actuation interlock.",
            "EOUSAGE",
            "}",
            "",
            "run_fixture() {",
            ...
            "",
            "trial_id() {",
            "  if [[ -n \"${QWENGUARD_TRIAL_ID:-}\" ]]; then",
            "    printf '%s\\n' \"${QWENGUARD_TRIAL_ID}\"",
            "  else",
            "    date -u '+trial-%Y%m%dT%H%M%SZ'",
            "  fi",
            "}",
            "",
            "run_record_success() {",
            "  local id",
            "  id=\"$(trial_id)\"",
            "  python3 -m scripts.record_qwenguard_trial \\",
            "    --trial-id \"${id}\" \\",
            "    --task-instruction \"${QWENGUARD_TASK}\" \\",
            "    --outcome success \\",
            "    --motion-executed true \\",
            "    --control-label \"${CONTROL_LABEL}\" \\",
            "    --confirm-operator-attestation \\",
            "    --trace-dir \"${TRIAL_TRACE_DIR}\" \\",
            "    --csv-out \"${TRIAL_CSV}\" \\",
            "    --report-out \"${TRIAL_REPORT_DIR}/${id}.json\"",
            "}",
            "",
            "run_record_failure() {",
            "  local id",
            ...
            "    --trial-id \"${id}\" \\",
            "    --task-instruction \"${QWENGUARD_TASK}\" \\",
            "    --outcome \"${QWENGUARD_TRIAL_OUTCOME:-missed_grasp}\" \\",
            "    --motion-executed true \\",
            "    --control-label \"${CONTROL_LABEL}\" \\",
            "    --confirm-operator-attestation \\",
            "    --trace-dir \"${TRIAL_TRACE_DIR}\" \\",
            ...
            "    fi",
            "  done",
            "  if [[ -d \"${TRIAL_TRACE_DIR}\" ]]; then",
            "    while IFS= read -r -d '' trace; do",
            "      python3 -m scripts.verify_trace \"${trace}\"",
            "    done < <(find \"${TRIAL_TRACE_DIR}\" -name '*.json' -type f -print0 | sort -z)",
            "  fi",
            "}",
            "",
            "case \"${PHASE}\" in",
...
def _evidence_template(task: str) -> dict[str, Any]:
    return {
        ...
        "operator_fill_required": {
            ...
            "trial_csv": "runs/physical/qwenguard_trials/trial_results.csv",
            "trial_recorder": "record-qwenguard-trial",
            "trial_attestation": "measured rows require --confirm-operator-attestation",
