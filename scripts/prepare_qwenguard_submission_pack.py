...
def _render_readme(*, repo_root: Path, files: dict[str, Path], task: str, repo_url: str) -> str:
    return "\n".join(
        [
            ...
            "> code validates and gates that proposal, a local SO-101 policy is",
            "> designed to act only when the gate allows it (physical execution",
            "> pending operator proof), cloud failure causes HOLD, and every decision",
...
def _render_demo_script(*, task: str) -> str:
    return "\n".join(
        [
            ...
            "",
            "## 1:00-1:45 Planned Local ACT Action",
            "",
            "- Record this segment only after a measured physical trace exists.",
            ...
            "",
            "- Measured `N/10` physical result, recorded only after trial traces exist.",
