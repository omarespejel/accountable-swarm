...
class WorldModelDashboardPackCliTests(TestCase):
    def test_dashboard_pack_verifies_fixture_hazard_world_model_timeline(self) -> None:
        ...
        self.assertIsNone(data["mission_choice"])
        self.assertEqual(data["mission_trace_summary_sha"], "")
    ...
    def test_dashboard_pack_threads_bounded_mission_choice_when_present(self) -> None:
        (ROOT / "runs").mkdir(parents=True, exist_ok=True)
        with TemporaryDirectory(dir=ROOT / "runs") as tmpdir:
            base = Path(tmpdir)
            trace_dir = base / "hazard_x"
        ...
        self.assertEqual(data["mission_choice"]["source"], "fixture")
        self.assertEqual(data["mission_choice"]["choice"]["mission"], "surround_hazard")
        self.assertEqual(data["mission_choice"]["choice"]["risk"], "cautious")
        self.assertEqual(len(data["mission_trace_summary_sha"]), 64)

    def test_dashboard_pack_rejects_rehashed_mission_choice_mismatch(self) -> None:
        (ROOT / "runs").mkdir(parents=True, exist_ok=True)
        with TemporaryDirectory(dir=ROOT / "runs") as tmpdir:
            base = Path(tmpdir)
            trace_dir = base / "hazard_x"
...
def _rewrite_mission_trace_choice(trace_path: Path, *, mission: str, risk: str) -> str:
    ...
    for event in trace.events:
        ...
        command["risk"] = risk
        rewritten = replace(event, command=command, prev_sha=prev_sha, sha256="").with_computed_sha()
        events.append(rewritten)
        prev_sha = rewritten.sha256
    rewritten_trace = DecisionTrace(
        run_id=trace.run_id,
