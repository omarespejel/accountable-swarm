...
class WorldModelDashboardRendererCliTests(TestCase):
    ...
    def test_renderer_builds_interactive_html_from_verified_dashboard_pack(self) -> None:
        ...
        self.assertIn("canvas.addEventListener('keydown'", html_text)
        self.assertIn("Use arrow keys to move the cursor", html_text)
        self.assertIn("fetch('/replan'", html_text)
        self.assertIn("AbortController", html_text)
        self.assertIn("replan timed out after 8s", html_text)
        self.assertIn("observations: currentObservations()", html_text)
        self.assertIn("obstaclePairsWithToggle", html_text)
        self.assertIn("Cannot place an obstacle on the hazard cell.", html_text)
        self.assertIn("Cannot place an obstacle on a live agent cell.", html_text)
        self.assertIn("applyReplanResponse", html_text)
        self.assertNotIn("preserveObservation", html_text)
