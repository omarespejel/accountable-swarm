...
class DemoRecordingPackCliTests(TestCase):
    ...
    def test_prepare_recording_pack_marks_bridge_manifest_not_go_when_timeline_missing(self) -> None:
        module = _load_module()
                    ...
                    json.dumps(
                        {
                            ...
                            "artifacts": {
                                "manifest": manifest_path.relative_to(ROOT).as_posix(),
                                "timeline_ndjson": (out_dir / "timeline.ndjson").relative_to(ROOT).as_posix(),
                            },
                            "dimos_probe": {
                                "runtime_outcome": "NARROW_CLAIM",
                                "source": {"checkout_provided": False},
        ...
        self.assertTrue(manifest["pass_conditions"]["dimos_bridge_command_succeeded"])
        self.assertFalse(manifest["pass_conditions"]["dimos_bridge_manifest_go"])

    def test_prepare_recording_pack_marks_bridge_manifest_not_go_when_manifest_not_go(self) -> None:
        module = _load_module()
